...
void new_constraint(bilia_dform_t * f,bilia_dform_t ** replace)
{
        ...
        case BILIA_DATA_EQ:
        {
                ...
                case BILIA_DATA_BAG:
                {
                        ...
                        {
                                bilia_var_t * var_int = bilia_vector_at(lvars, i);
                                if (!var_int->element  || var_int->vid == a0->vid || var_int->vty->kind != BILIA_TYP_INT || !var_int->used) continue;
                                bilia_dterm_t * zero = bilia_dterm_new();
                                zero->kind = BILIA_DATA_INT;
                                zero->typ = BILIA_TYP_INT;
                                zero->p.value = 0;
                ...
                case BILIA_DATA_BAGMIN:
                {
                        if(!bilia_option_get_nr()) {
                                ...
                                for (uint_t i = 0; i < BILIA_VECTOR_SIZE(lvars); i++)
                                {
                                        ...
                                        zero->typ = BILIA_TYP_INT;
                                        zero->p.value = 0;
                                        bilia_dform_array_push(and2->p.bargs, bilia_dform_new_eq(matching_w(var_int->vname, var_bag_in->vname), zero));
                                }
                ...
                if(!int_l->extremum){
                  ...
                  *replace = geq;
                  ...
                  for (uint_t i = 0; i < BILIA_VECTOR_SIZE(lvars); i++)
                  {
                          ...
                          zero->typ = BILIA_TYP_INT;
                          zero->p.value = 0;
                          bilia_dform_array_push(and->p.bargs, bilia_dform_new_eq(matching_w(var_int->vname, bag_r->vname), zero));
                  }
                ...
                if(!int_l->extremum){
                  ...
                  *replace = geq;
                  ...
                  for (uint_t i = 0; i < BILIA_VECTOR_SIZE(lvars); i++)
                  {
                          ...
                          bilia_dterm_array_push(neq->p.targs,matching_w(var_int->vname, bag_r->vname));
