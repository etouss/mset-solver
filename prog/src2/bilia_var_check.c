...
bool bilia_term_is_extremum(bilia_dterm_t *term){
        bool res = false;
        if(term->kind == BILIA_DATA_BAGMAX ||term->kind == BILIA_DATA_BAGMIN) {
                return true;
        }
        if(term->args != NULL)
                for (size_t i = 0; i < BILIA_VECTOR_SIZE(term->args); i++) {
                        res = res || bilia_term_is_extremum(bilia_vector_at(term->args,i));
                }
        else if(term->kind == BILIA_DATA_VAR) {
                bilia_var_t * var = bilia_vector_at(bilia_entl_get_var_array(),term->p.sid);
                res = res || var->extremum;
        }
        return res;
}
...
int bilia_term_apply_extremum(bilia_dterm_t * term){
        int res = 0;
        if(term->kind == BILIA_DATA_PLUS || term->kind == BILIA_DATA_MINUS)
        {
          fprintf (stderr,
                   "Error of level you can not mix extremum and integer variables.\n");
          bilia_dterm_fprint (stderr, bilia_entl_get_var_array(), term,0);
          exit(0);
        }
        if(term->args != NULL)
                for (size_t i = 0; i < BILIA_VECTOR_SIZE(term->args); i++)
                        res += bilia_term_apply_extremum(bilia_vector_at(term->args,i));
        else if(term->kind == BILIA_DATA_VAR) {
                bilia_var_t * var = bilia_vector_at(bilia_entl_get_var_array(),term->p.sid);
                if(var->vty->kind == BILIA_TYP_INT && !var->extremum) {
                        var->extremum = true;
                        res++;
                }
        }
...
int bilia_var_extremum(bilia_dform_t * form){
        int nb_switch = 0;
        if(form->kind == BILIA_DATA_AND || form->kind == BILIA_DATA_OR) {
                /*Propagate the nnf*/
                for (uint_t i = 0; i< BILIA_VECTOR_SIZE(form->p.bargs); i++)
                        nb_switch += bilia_var_extremum(bilia_vector_at(form->p.bargs,i));
                return nb_switch;
        }
        if(form->kind == BILIA_DATA_EQ || form->kind == BILIA_DATA_NEQ || form->kind == BILIA_DATA_LE || form->kind == BILIA_DATA_GE || form->kind == BILIA_DATA_GT \
           || form->kind == BILIA_DATA_LT || form->kind == BILIA_DATA_INN  || form->kind == BILIA_DATA_NIN  || form->kind == BILIA_DATA_IN  || form->kind == BILIA_DATA_NI \
           || form->kind == BILIA_DATA_SUBSETEQ || form->kind == BILIA_DATA_NSUBSETEQ) {
                ...
                for (uint_t i = 0; i < BILIA_VECTOR_SIZE(form->p.targs); i++)
                        is_extremum = is_extremum || bilia_term_is_extremum(bilia_vector_at(form->p.targs,i));
                if(is_extremum) {
                        // if(form->kind == BILIA_DATA_INN  || form->kind == BILIA_DATA_NIN  || form->kind == BILIA_DATA_IN  || form->kind == BILIA_DATA_NI \
                        //    || form->kind == BILIA_DATA_SUBSETEQ || form->kind == BILIA_DATA_NSUBSETEQ)
                        // {
                        //   fprintf (stderr,
                        //            "Error of level you can not mix extremum and integer variables 2.\n");
                        //   bilia_dform_fprint (stderr, bilia_entl_get_var_array(), form,0);
                        //   exit(0);
                        // }
                        for (uint_t i = 0; i < BILIA_VECTOR_SIZE(form->p.targs); i++)
                                nb_switch += bilia_term_apply_extremum(bilia_vector_at(form->p.targs,i));
                }
        }
        return nb_switch;
}

void bilia_var_union_find(){
        ...
        while(42){
          ...
          for (size_t i = 0; i < BILIA_VECTOR_SIZE(rf); i++)
            {
              res += bilia_var_extremum(bilia_vector_at(rf,i));
              //res += bilia_var_extremum(bilia_vector_at(rf,i));
            }
          for (size_t i = 0; i < BILIA_VECTOR_SIZE(af); i++)
            {
              res += bilia_var_extremum(bilia_vector_at(af,i));
              //res += bilia_var_extremum(bilia_vector_at(af,i));
            }
          if(res == 0)break;
        }
        if(bilia_option_get_optim_ele())
          while(42){
            ...
            for (size_t i = 0; i < BILIA_VECTOR_SIZE(rf); i++)
              {
                res += bilia_var_element(bilia_vector_at(rf,i));
                //res += bilia_var_extremum(bilia_vector_at(rf,i));
              }
            for (size_t i = 0; i < BILIA_VECTOR_SIZE(af); i++)
              {
                res += bilia_var_element(bilia_vector_at(af,i));
                //res += bilia_var_extremum(bilia_vector_at(af,i));
              }
            if(res == 0)break;
          }
